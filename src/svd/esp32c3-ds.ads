...
package ESP32C3.DS is
   ...
   --  memory that stores Y
   type Y_MEM_Registers is array (0 .. 511) of HAL.Byte;

   --  memory that stores M

   --  memory that stores M
   type M_MEM_Registers is array (0 .. 511) of HAL.Byte;

   --  memory that stores Rb

   --  memory that stores Rb
   type RB_MEM_Registers is array (0 .. 511) of HAL.Byte;

   --  memory that stores BOX

   --  memory that stores BOX
   type BOX_MEM_Registers is array (0 .. 47) of HAL.Byte;

   --  memory that stores X

   --  memory that stores X
   type X_MEM_Registers is array (0 .. 511) of HAL.Byte;

   --  memory that stores Z

   --  memory that stores Z
   type Z_MEM_Registers is array (0 .. 511) of HAL.Byte;
   ...
   subtype QUERY_KEY_WRONG_QUERY_KEY_WRONG_Field is HAL.UInt4;
